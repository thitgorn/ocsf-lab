 ...
 * is outside the synchronized block and is preceded by a join that guarantees
...
public abstract class AbstractServer implements Runnable {
	...
	/**
	 * The factory used to create new connections to clients. Is null by
	 * default, meaning that regular <code>ConnectionToClient</code> instances
	 * will be created. Added in version 2.3
	 */
	...
	/**
	 * Causes the server to stop accepting new connections, but doesn't affect
	 * currently connect clients.
	 */
	...
	final public void close() throws IOException {
		...
		try {
			...
			try {
				connectionListener.join(); // Wait for the end of listening
											// thread.
	...
	/**
	 * Sets the connection factory. Once set, this one will be used in the
	 * creation of new <code>ConnectionToClient</code> instances. The call to
	 * this method is optional; if not called Then regular
	 * <code>ConnectionToClient</code> instances are created. Added in version
	 * 2.3
	 *
	 * @param factory
						...
						if (!readyToStop) // added in version 2.2
						{
							if (connectionFactory == null) {
								...
								connectionFactory.createConnection(
										this.clientThreadGroup, clientSocket,
										this);
							}
						}
					}
	 ...
	 * <p>
	 * This method is designed for use by the framework only. Applications
	 * should not normally invoke it.
	 *
	 * @param msg
	 *            the message sent.
	 * @param client
	 ...
	 * client's thread except for
	 * <code>ClassNotFoundException<\code>s received when an object of
	 * unknown class is received and for the <code>RuntimeException</code>s that
	 * can be thrown by the message handling method implemented by the user.
	 *
	 * @param client
